...
fn main() {
    ...
    // --- Compile (graph search) ---
    compile_and_run!(cx, |rt, backend| {
        let compile_s = compile_start.elapsed().as_secs_f64();
        eprintln!("[luminal] compiled in {compile_s:.2}s ({backend})");
        ...
        // Re-set all inputs+weights since the runtime consumes buffers.
        for (i, wt) in all_weights.iter().enumerate() {
            let n = wt
                .dims()
                .iter()
        ...
        let result = serde_json::json!({
            ...
            "model": model_name,
            "device": backend.to_lowercase(),
            "gpu_name": backend.to_lowercase(),
            "backend": backend,
            "timings": {
